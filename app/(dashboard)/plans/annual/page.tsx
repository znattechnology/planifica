...
export default function AnnualPlansPage() {
  ...
  useEffect(() => {
    async function fetchPlans() {
      try {
        const res = await fetchWithAuth(`${API_ROUTES.PLANS}?type=ANNUAL`)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold tracking-tight sm:text-3xl">Dosificação Anual</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            Gira os planos anuais gerados a partir das dosificações.
          </p>
        </div>
        <Button className="bg-accent text-accent-foreground hover:bg-accent/90" asChild>
          <Link href={ROUTES.PLANS_NEW}>
            <Plus className="mr-2 h-4 w-4" />
            Gerar Dosificação Anual
          </Link>
        </Button>
      </div>
      ...
      {isEmpty && (
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-border/60 px-4 py-16 text-center">
          ...
          <h3 className="mt-4 text-lg font-semibold">Nenhum plano anual</h3>
          <p className="mt-2 max-w-sm text-sm text-muted-foreground">
            Gere o seu primeiro plano anual a partir de uma dosificação. A IA criará um plano
            completo com objectivos, competências e temas.
          </p>
      ...
      {!isEmpty && (
        <div className="space-y-3">
          {filtered.map((plan) => {
            ...
            const topicsCount = plan.content?.topics?.length ?? 0
            const objectivesCount = plan.content?.objectives?.length ?? 0
              ...
              >
                ...
                <div className="flex items-center gap-3 p-4 sm:gap-4 sm:p-5">
                  ...
                  <div className="min-w-0 flex-1">
                    <h3 className="truncate text-sm font-semibold sm:text-base">{plan.title}</h3>
          ...
          <div className="flex items-start gap-3 sm:items-center">
            ...
            <div>
              <h3 className="text-sm font-semibold sm:text-base">
                Dosificação Anual → Trimestral → Quinzenal → Plano de Aula
              </h3>
              <p className="mt-0.5 text-xs text-muted-foreground sm:text-sm">
                Após aprovar um plano anual, gere automaticamente planos trimestrais, quinzenais e de aula em cascata.
              </p>
            </div>
          </div>
